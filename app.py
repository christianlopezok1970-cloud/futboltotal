                ...
                with st.expander(f"{j[0]}"):
                    ...
                    if modo == "titular":
                        ...
                        if not st.session_state.get(confirm_key, False):
                            ...
                            if c_v1.button("✅ SI", key=f"si_v_{id_jugador}", type="primary", use_container_width=True):
                                ...
                                st.rerun()
                            if c_v2.button("❌ NO", key=f"no_v_{id_jugador}", use_container_width=True):
                                st.session_state[confirm_key] = False
                                st.rerun()
...
if not st.session_state.get('conf_fichar', False):
    ...
    st.warning("¿Quieres gastar 50 🪙 en un fichaje aleatorio?")
    cf1, cf2 = st.columns(2)
    if cf1.button("✅ CONFIRMAR COMPRA", key="fichar_si", type="primary", use_container_width=True):

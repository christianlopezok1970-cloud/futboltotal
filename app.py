...
# --- 3. SISTEMA DE USUARIO Y PRESUPUESTO ---
with st.sidebar:
...
u_id, presupuesto_actual = user_data[0]

# --- 4. MERCADO (BLOQUEO DE DUPLICADOS Y DESCUENTO REAL) ---
df_j = cargar_jugadores()
st.sidebar.metric("Presupuesto Disponible", f"€ {presupuesto_actual:,.0f}")
    ...
    if seleccion:
        ...
        if st.button("CONFIRMAR OPERACIÓN"):
            ...
            if existe:
                ...
                # El rerun forzará a la sección 3 a leer el nuevo presupuesto de la DB
                st.rerun()
